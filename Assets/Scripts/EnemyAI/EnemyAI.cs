...
public class EnemyAI : MonoBehaviour
{
    ...
    private void FieldOfViewCheck()
    {
        ...
        {
            Transform whatIsPlayer = rangeChecks[0].transform;
            Vector3 directionToTarget = (whatIsPlayer.position - transform.position).normalized;

            if (Vector3.Angle(transform.forward, directionToTarget) < angle / 2)
            {
                float distanceToTarget = Vector3.Distance(transform.position, whatIsPlayer.position);
